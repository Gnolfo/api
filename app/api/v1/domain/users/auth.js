 ...
 */
module.exports = {
  TOKEN_EXPIRATION_DAYS: 7, // Days from now that a new token will expire

  ISSUER: 'API', // 'Issuer' of token

  /**
   * Create a JWT token for the passed in user. Argument can be a user
   * model instance or a user id
  ...
  /**
   * Refresh the expiration date of the passed in token. Returns a new token
   * for a valid token, or false for an expired token
   * @param  {string} token JWT token to refresh
   * @return {mixed}  Returns a new token string with a refreshed expiration date
   *                  if valid, otherwise returns false
   */
